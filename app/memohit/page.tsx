...
import { Lock, LogOut, RefreshCw, Shield, Search, AlertTriangle, Gift } from "lucide-react"
...
export default function AdminPanel() {
  ...
  const [activeTab, setActiveTab] = useState<"stores" | "withdrawals">("stores")
  ...
  const loadWithdrawals = useCallback(async () => {
    ...
    try {
      const { data } = await supabase
        ...
        .select("id, user_id, amount, upi_id, status, admin_note, created_at")
        .order("created_at", { ascending: false })
      setWithdrawals((data ?? []) as WithdrawalAdmin[])
    } finally {
  ...
  const handleWithdrawalStatus = async (id: string, status: string) => {
    ...
    await supabase
      .from("withdrawal_requests")
      .update({ status, admin_note: adminNote[id] ?? null, updated_at: new Date().toISOString() })
      .eq("id", id)
  ...
  useEffect(() => {
    if (isAuthenticated) loadWithdrawals()
  }, [isAuthenticated, loadWithdrawals])
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 mb-6 shadow-2xl">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-3">
              <button
                onClick={() => { loadStores(); loadWithdrawals() }}
                disabled={loading}
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === "stores" ? "bg-blue-600 text-white" : "bg-white/10 text-gray-300 hover:bg-white/20"}`}
          >
            Stores ({stores.length})
          </button>
          <button
            onClick={() => setActiveTab("withdrawals")}
            className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === "withdrawals" ? "bg-purple-600 text-white" : "bg-white/10 text-gray-300 hover:bg-white/20"}`}
          >
          ...
          <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl overflow-hidden shadow-2xl">
            ...
            {withdrawalsLoading ? (
              ...
              <div className="divide-y divide-white/10">
                {withdrawals.map((w) => (
                  <div key={w.id} className="p-4 flex flex-col md:flex-row md:items-center gap-4">
                    <div className="flex-1">
                      <p className="text-white font-semibold">₹{w.amount.toFixed(2)}</p>
                      <p className="text-gray-400 text-sm">UPI: {w.upi_id}</p>
                      <p className="text-gray-500 text-xs">{w.user_id} · {new Date(w.created_at).toLocaleString()}</p>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <input
                        ...
                        className="text-xs px-2 py-1 bg-white/10 border border-white/20 rounded text-white placeholder-gray-500 focus:outline-none w-40"
                      />
